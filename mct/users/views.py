...
from mct import db
from mct.models import User, BlogPost, BlogCategory
from mct.users.forms import RegistrationForm, LoginForm, UpdateUserForm
from mct.main.forms import BlogSearchForm
...
#ユーザー用登録ページ
@users.route('/users_register', methods=['GET','POST'])
def users_register():
    form = RegistrationForm()
    if form.validate_on_submit():
        # session['email'] = form.email.data
        ...
        db.session.commit()
        flash('ユーザーが登録されました')
        return redirect(url_for('users.rg_page'))
    return render_template('users/register.html', form=form)

@users.route('/rg_page', methods=['GET', 'POST'])
def rg_page():
    return render_template('users/rg_page.html')
